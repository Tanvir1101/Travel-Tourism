				...
				<table id = "table">
					...
					@foreach ($allUserList as $user)
						<tr>
							...
							<td>{{$user->email}}</td>
							<td>{{$user->status}}</td>
							<td><a href="{{route('adminUser.userDetails', [$user->id])}}"><i class="fa fa-info-circle" aria-hidden="true"></i></a>
							<a href="{{route('adminUser.userDelete', [$user->id])}}"><i class="fa fa-trash" aria-hidden="true"></i></a></td>
						</tr>
					@endforeach
